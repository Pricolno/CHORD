...
from preprocess_data import load_np, SEMITONES, merge_chords, transform_into_dataset_of_modes, transform_into_dataset_of_shifts
...
def train():
    ...
    for i, (train_indices, test_indices) in enumerate(kf.split(composition_idx_unique)):
        ...
        train_mask = [True if composition_idx_ in train_compositions else False for composition_idx_ in
                      composition_idx]
        ...
        root_model = CatBoostClassifier(**PARAMS_ROOT, logging_level="Silent")
        mode_model = CatBoostClassifier(**PARAMS_MODE, logging_level="Silent")
        nochord_model = CatBoostClassifier(**PARAMS_NOCHORD, logging_level="Silent")

        root_model.fit(X_train_shifts_merged, y_train_chord_merged)
        mode_model.fit(X_train_modes_merged, y_train_modes_merged)
        nochord_model.fit(X_train_merged, y_train_nochord_merged)

        test_mask = [True if composition_idx_ in test_compositions else False for composition_idx_ in composition_idx]

        X_test = X[test_mask]
        y_test = labels[test_mask]

        X_test_merged, y_test_merged = merge_chords(X_test, y_test)

        X_test_modes_merged, y_test_modes_merged = transform_into_dataset_of_modes(X_test_merged, y_test_merged)
    ...
    #
    # X = np.concatenate((X_train_merged, X_test_merged))
    # y = np.concatenate((y_train_merged, y_test_merged))
    # print(X.shape)
    # print(y.shape)
    #
    ...
    # y_test_chord_merged = load_np("y_test_chord_merged")
    #
    # X_shifts = np.concatenate((X_train_shifts_merged, X_test_shifts_merged))
    # y_chord = np.concatenate((y_train_chord_merged, y_test_chord_merged))
    #
    ...
    # y_test_modes_merged = load_np("y_test_modes_merged")
    #
    # X_modes = np.concatenate((X_train_modes_merged, X_test_modes_merged))
    # y_modes = np.concatenate((y_train_modes_merged, y_test_modes_merged))
    #
    # print(X_modes.shape)
    # print(y_modes.shape)
