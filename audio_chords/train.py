...
if USE_GPU:
    PARAMS_ROOT.update(GPU_SETTINGS)
    PARAMS_MODE.update(GPU_SETTINGS)
    PARAMS_NOCHORD.update(GPU_SETTINGS)


def get_accuracy(model_roots, model_mode, model_nochord,
                 X_merged, X_shifts, X_modes, y):
    preds_root = model_roots.predict_proba(X_shifts, verbose=True)
    if X_shifts.shape[1] != 2 * SEMITONES:
        X_shifts = X_shifts[:, :2 * SEMITONES]
...
def train():
    ...
    # TRAIN ACCURACY: 0.9061677470045177
    # TEST ACCURACY: 0.8503732809430256
