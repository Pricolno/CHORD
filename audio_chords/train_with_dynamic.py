...
LOGGER_DIR = os.path.join(DATA_DIR, 'loggers')
NPY_DIR = os.path.join(DATA_DIR, 'castom_npy')

import logging
my_logger = logging.getLogger('naumtsev_logger')
my_logger.setLevel(logging.INFO) # Устанавливаем уровень логирования

# LOGGER_TITLE = 'harmony_with_neg_app.log'
LOGGER_TITLE = 'harmony_without_neg.log'


LOGGER_FILE_PATH = os.path.join(LOGGER_DIR, LOGGER_TITLE)
app_file_handler = logging.FileHandler(LOGGER_FILE_PATH, mode='w')
# Создаем форматтер и добавляем его в хэндлер
...
print(f'{SEP}\nLogs saving in {LOGGER_FILE_PATH}\n{SEP}')
...
if __name__ == '__main__':
    ...
    for i, (train_indices, test_indices) in enumerate(kf.split(composition_idx_unique)):
        if i == args.fold_id:
            my_logger.info(f"{i=}| {len(train_indices)=} | {len(test_indices)=}")
            ...
            # X_negative_train_merged, y_negative_train_merged = create_negative_root_chord(X_train_merged, X_train_merged)
            ...
            for test_composition in tqdm(test_compositions):
                ...
                folder_dir = f'{i:02}_test_folder'
                experiment_title = 'woNochord_negSamples'
                result_save_dir = os.path.join(NPY_DIR, folder_dir)
                os.makedirs(result_save_dir, exist_ok=True)
                labels_file_path = os.path.join(result_save_dir, f'{experiment_title}_labeling.json')
                with open(labels_file_path, 'w') as f:
                    json.dump(labeling, f)

            my_logger.info(f'Fold {i}')
            my_logger.info(f'Total score {score_sum / len(test_compositions)}')
